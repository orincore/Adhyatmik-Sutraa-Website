...
const nextConfig = {
  images: {
    ...
    // original size, which PageSpeed flagged as tens of MB of avoidable
    // image weight on a single page. `sharp` is present in node_modules
    // (Next's optimizer auto-detects and uses it), so re-enabling this lets
    // Next resize/recompress/reformat on the fly instead. Verify on staging
