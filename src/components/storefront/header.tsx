...
export function Header() {
  ...
  const iconCls = "text-white/80 hover:text-[#EEC6BC] hover:bg-white/10";
  const navCls = "text-white/85 hover:text-[#EEC6BC] hover:bg-white/10 uppercase tracking-[0.14em] text-[11.5px] font-semibold";
  const borderCls = "border-white/15";
  ...
  return (
    <>
      ...
      >
        <div
          className="pointer-events-auto w-full bg-gradient-to-r from-[#240429] via-[#35093C] to-[#240429] backdrop-blur-md border-b border-[#D89E2E]/30 shadow-[0_6px_30px_rgba(36,4,41,0.45)]"
        >
          <div className="mx-auto max-w-7xl flex h-16 items-center justify-between px-4 sm:px-6">
            ...
            <nav className="hidden lg:flex items-center gap-0.5">
              {navLinks.map((link) => (
                <Link key={link.href} href={link.href} className={`px-3 py-2 rounded-full transition-all duration-200 ${navCls}`}>
                  {link.label}
                </Link>
              ))}
            </nav>
            ...
            <div className="flex items-center gap-1">
              ...
              >
                ...
                {mounted && itemCount > 0 && (
                  <span className="absolute -top-0.5 -right-0.5 h-4 w-4 rounded-full bg-[#FD4380] text-[10px] font-bold text-white flex items-center justify-center shadow">
              ...
              {mounted && !loading && (
                customer ? (
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      className="hidden md:flex items-center gap-1.5 h-8 px-3.5 rounded-full bg-[#FD4380] hover:bg-[#e42a6c] text-white text-xs font-semibold transition shadow-[0_2px_12px_rgba(253,67,128,0.4)]"
                      onClick={() => router.push("/account")}
                    >
                  ...
                  <div className="flex items-center gap-1.5">
                    <button
                      className={`h-8 px-3 rounded-full text-xs font-semibold transition ${iconCls}`}
                      onClick={() => router.push("/login")}
                    >
                      Sign In
                    </button>
                    <button
                      className="h-8 px-3.5 rounded-full bg-[#FD4380] hover:bg-[#e42a6c] text-white text-xs font-semibold transition shadow-[0_2px_12px_rgba(253,67,128,0.4)]"
          ...
          <div className={`overflow-hidden transition-all duration-300 ${searchOpen ? "max-h-[420px] opacity-100" : "max-h-0 opacity-0"}`}>
            <div className="mx-auto max-w-7xl px-4 sm:px-6 pb-3 pt-1 border-t border-white/15">
              <div className="flex items-center gap-2 bg-white/10 rounded-full px-4 py-2 border border-white/15 focus-within:border-[#D89E2E]/50 transition">
              ...
              {(searchLoading || searchResults.length > 0 || searchError || (searchQuery && !searchLoading)) && (
                <div className="mt-2 max-h-80 overflow-y-auto rounded-2xl bg-white/95 backdrop-blur-sm border border-[#D89E2E]/20 shadow-xl divide-y divide-black/5">
                  {searchLoading && (
                  ...
                  {!searchLoading && searchResults.map((r) => {
                    ...
                    return (
                      <button
                        key={`${r.kind}-${r.id}`}
                        className="flex w-full items-center gap-3 px-4 py-2.5 hover:bg-purple-50/80 transition text-left"
            ...
            <div className={`px-4 pb-4 pt-1 border-t space-y-1 ${borderCls}`}>
              {navLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className={`block px-3 py-2.5 rounded-xl transition ${navCls}`}
