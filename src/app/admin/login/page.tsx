...
export default function AdminLoginPage() {
  ...
  // in lib/auth.ts) — the browser sends it to crm.adhyatmiksutraa.com on
  // its own, so getting there is just a plain navigation now. No token is
  // minted or put in the URL: there's nothing left for a stray/forwarded
  // link to leak and nothing for a flaky backend call to fail partway
  // through, which is what the old mint-a-token-and-redirect handoff was
  // prone to.
  //
  ...
  const getRedirectUrl = () => {
    ...
    try {
      ...
      const isTrusted = host === "adhyatmiksutraa.com" || host.endsWith(".adhyatmiksutraa.com") || host === "localhost";
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, #240429 0%, #35093C 50%, #542b54 100%)" }}
      >
        <div className="absolute -top-20 -left-20 w-80 h-80 rounded-full opacity-10" style={{ background: "radial-gradient(circle, #ff94ba, transparent)" }} />
        <div className="absolute -bottom-28 -right-12 w-[420px] h-[420px] rounded-full opacity-10" style={{ background: "radial-gradient(circle, #fe6a9e, transparent)" }} />
      ...
      <div className="flex-1 flex items-center justify-center p-6 sm:p-10 pt-[110px] lg:pt-10 bg-gray-50 overflow-y-auto">
        <div className="w-full max-w-sm space-y-8">
          ...
          <div className="space-y-1.5">
            <h1 className="text-3xl text-[#35093C] font-cormorant" style={{ fontWeight: 600 }}>
              Admin sign in
            </h1>
          ...
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            <button
              ...
              style={{ background: loading ? "#374151" : "linear-gradient(135deg, #35093C 0%, #e42a6c 100%)" }}
            >
              {loading ? (
